...
export default class ClassroomIndexController extends Controller {
  ...
  init() {
    ...
    this.eventBus.on(
      this.eventBus.videoActions.onDeleted,
      this,
      this.onVideoDeleted
    );
    this.eventBus.on(
    ...
    this.eventBus.on(
      this.eventBus.topicActions.onAdded,
      this,
      this.onTopicAdded
    );
    this.eventBus.on(
      this.eventBus.topicActions.onDeleted,
      this,
      this.onTopicDeleted
    );
    this.eventBus.on(
      this.eventBus.topicActions.onEdited,
      this,
      this.onTopicEdited
    );
  }
  ...
  onVideoAdded(video) {
    let topicIdx = this.topics.findIndex(
      (topic) => topic.id === video.classroom_topic_id
    );
    if (topicIdx === -1) {
      return;
    }
  ...
  onVideoEdited(video) {
    ...
    let videoIdx = this.topics[topicIdx].videos.findIndex(
      (v) => v.id === video.id
    );
    if (videoIdx === -1) {
      return;
